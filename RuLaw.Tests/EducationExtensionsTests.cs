...
namespace RuLaw
{
  ...
  {
    /// <summary>
    ///   <para>Performs testing of <see cref="EducationExtensions.Institution(IEnumerable{Education}, string)"/> method.</para>
    /// </summary>
    [Fact]
    public void Institution_Method()
    {
      Assert.Throws<ArgumentNullException>(() => EducationExtensions.Institution(null, "institution"));
      Assert.Throws<ArgumentNullException>(() => Enumerable.Empty<Education>().Institution(null));
      Assert.Throws<ArgumentException>(() => Enumerable.Empty<Education>().Institution(string.Empty));

      Assert.False(Enumerable.Empty<Education>().Institution("institution").Any());
    ...
    /// <summary>
    ///   <para>Performs testing of <see cref="EducationExtensions.Year(IEnumerable{Education}, short?, short?)"/> method.</para>
    /// </summary>
    [Fact]
    public void Year_Method()
    {
      ...
      Assert.False(Enumerable.Empty<Education>().Year().Any());
