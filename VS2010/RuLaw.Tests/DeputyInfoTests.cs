...
namespace RuLaw
{
  ...
  public sealed class DeputyInfoTests : UnitTestsBase<DeputyInfo>
  {
    ...
    public void Json()
    {
      ...
      this.TestJson(
        new DeputyInfo
        {
          Id = 1,
          Active = true,
          Activities = new List<DeputyActivity> { new DeputyActivity { Name = "activity.name", CommitteeId = 1 } },
          BirthDate = DateTime.MaxValue,
          Degrees = new List<string> { "degree" },
          Educations = new List<Education> { new Education { Year = 2000 } },
          FactionId = 2,
    ...
    public void Constructors()
    {
      ...
      Assert.Equal(string.Empty, deputy.FullName);
    ...
    public void Activities_Property()
    {
      ...
      deputy.Activities.Add(activity);
      Assert.True(ReferenceEquals(activity, deputy.Activities.Single()));
      deputy.Activities.Remove(activity);
      Assert.False(deputy.Activities.Any());
    }
    ...
    public void Degrees_Property()
    {
      var deputy = new DeputyInfo();
      Assert.False(deputy.Degrees.Any());
      deputy.Degrees.Add("degree");
      Assert.Equal("degree", deputy.Degrees.Single());
      deputy.Degrees.Remove("degree");
      Assert.False(deputy.Degrees.Any());
    }
    ...
    public void Educations_Property()
    {
      ...
      deputy.Educations.Add(education);
      Assert.True(ReferenceEquals(education, deputy.Educations.Single()));
      deputy.Educations.Remove(education);
      Assert.False(deputy.Educations.Any());
    }
    ...
    /// <summary>
    ///   <para>Performs testing of <see cref="DeputyInfo.FullName"/> property.</para>
    /// </summary>
    [Fact]
    public void FullName_Property()
    {
    ...
    public void Ranks_Property()
    {
      var deputy = new DeputyInfo();
      Assert.False(deputy.Ranks.Any());
      deputy.Ranks.Add("rank");
      Assert.Equal("rank", deputy.Ranks.Single());
      deputy.Ranks.Remove("rank");
      Assert.False(deputy.Ranks.Any());
    }
    ...
    public void Regions_Property()
    {
      var deputy = new DeputyInfo();
      Assert.False(deputy.Regions.Any());
      deputy.Regions.Add("region");
      Assert.Equal("region", deputy.Regions.Single());
      deputy.Regions.Remove("region");
      Assert.False(deputy.Regions.Any());
    }
    ...
    /// <summary>
    ///   <para>Performs testing of <see cref="DeputyInfo.CompareTo(DeputyInfo)"/> method.</para>
    /// </summary>
    [Fact]
    ...
    ///   <list type="bullet">
    ///     <item><description><see cref="DeputyInfo.Equals(DeputyInfo)"/></description></item>
