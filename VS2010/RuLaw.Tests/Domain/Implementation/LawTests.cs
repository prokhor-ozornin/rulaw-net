...
namespace RuLaw
{
  ...
  public sealed class LawTests : UnitTestsBase<Law>
  {
    ...
    public void Constructors()
    {
      ...
      Assert.False(law.CommitteesOriginal.ProfileOriginal.Any());
      Assert.False(law.CommitteesOriginal.Profile.Any());
      Assert.Null(law.CommitteesOriginal.Responsible);
      Assert.False(law.CommitteesOriginal.SoExecutorOriginal.Any());
      Assert.False(law.CommitteesOriginal.SoExecutor.Any());
      Assert.Equal(default(DateTime), law.Date);
      Assert.Null(law.LastEvent);
      Assert.Null(law.Name);
      Assert.Null(law.Number);
      Assert.False(law.SubjectOriginal.DepartmentsOriginal.Any());
      Assert.False(law.SubjectOriginal.Departments.Any());
      Assert.False(law.SubjectOriginal.DeputiesOriginal.Any());
      Assert.False(law.SubjectOriginal.Deputies.Any());
