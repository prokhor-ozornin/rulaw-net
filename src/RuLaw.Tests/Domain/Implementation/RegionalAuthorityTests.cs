...
namespace RuLaw
{
  ...
  public sealed class RegionalAuthorityTests : UnitTestsBase<RegionalAuthority>
  {
    ...
    public void Json()
    {
      TestJson(new RegionalAuthority(), new { id = 0, isCurrent = false, startDate = default(DateTime) });
      TestJson(
    ...
    public void Xml()
    {
      TestXml(new RegionalAuthority(), "regionalOrgan", new { id = 0, isCurrent = false, startDate = default(DateTime).ISO8601() });
      TestXml(
        new RegionalAuthority
        {
          Id = 1,
