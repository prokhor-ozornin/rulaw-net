...
namespace RuLaw
{
  ...
  public sealed class DeputyInfoTests : UnitTestsBase<DeputyInfo>
  {
    ...
    public void Constructors()
    {
      ...
      Assert.Empty(deputy.ActivitiesOriginal);
      Assert.Empty(deputy.Activities);
      Assert.Equal(default(DateTime), deputy.BirthDate);
      Assert.Empty(deputy.DegreesOriginal);
      Assert.Empty(deputy.Degrees);
      Assert.Empty(deputy.EducationsOriginal);
      Assert.Empty(deputy.Educations);
      ...
      Assert.Empty(deputy.RanksOriginal);
      Assert.Empty(deputy.Ranks);
      Assert.Empty(deputy.RegionsOriginal);
      Assert.Empty(deputy.Regions);
