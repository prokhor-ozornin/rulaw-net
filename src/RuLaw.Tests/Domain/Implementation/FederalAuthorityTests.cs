...
namespace RuLaw
{
  ...
  public sealed class FederalAuthorityTests : UnitTestsBase<FederalAuthority>
  {
    ...
    public void Json()
    {
      TestJson(new FederalAuthority(), new { id = 0, isCurrent = false, startDate = default(DateTime) });
      TestJson(
    ...
    public void Xml()
    {
      TestXml(new FederalAuthority(), "federalOrgan", new { id = 0, isCurrent = false, startDate = default(DateTime).ISO8601() });
      TestXml(
        new FederalAuthority
        {
          Id = 1,
