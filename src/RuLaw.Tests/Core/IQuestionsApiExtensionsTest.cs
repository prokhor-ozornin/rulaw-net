...
public sealed class IQuestionsApiExtensionsTest : IDisposable
{
  ...
  ///   <list type="bullet">
  ///     <item><description><see cref="IQuestionsApiExtensions.Search(IQuestionsApi, Action{IQuestionsApiRequest}?, CancellationToken)"/></description></item>
  ///     <item><description><see cref="IQuestionsApiExtensions.Search(IQuestionsApi, out IQuestionsSearchResult?, Action{IQuestionsApiRequest}?, CancellationToken)"/></description></item>
  ///   </list>
  /// </summary>
  [Fact]
  public void Search_Methods()
  {
    ...
    {
      AssertionExtensions.Should(() => IQuestionsApiExtensions.Search(null!)).ThrowExactlyAsync<ArgumentNullException>().Await();
      AssertionExtensions.Should(() => IQuestionsApiExtensions.Search(Api.Questions, null, Cancellation)).ThrowExactlyAsync<TaskCanceledException>().Await();

    }

    using (new AssertionScope())
    {
      AssertionExtensions.Should(() => IQuestionsApiExtensions.Search(null!, out _)).ThrowExactly<ArgumentNullException>();
      AssertionExtensions.Should(() => IQuestionsApiExtensions.Search(null!, out _)).ThrowExactly<ArgumentNullException>();
      AssertionExtensions.Should(() => Api.Questions.Search(out _, null, Cancellation)).ThrowExactly<TaskCanceledException>();
      AssertionExtensions.Should(() => Api.Questions.Search(out _, null, Cancellation)).ThrowExactly<TaskCanceledException>();

      Api.Questions.Search(out var result, request => request.FromDate(new DateTimeOffset(year: 2013, month: 1, day: 1, hour: 0, minute: 0, second: 0, TimeSpan.Zero)).ToDate(new DateTimeOffset(year: 2013, month: 12, day: 31, hour: 0, minute: 0, second: 0, TimeSpan.Zero)).Name("образование").PageSize(PageSize.Five).Page(2)).Should().BeTrue();
