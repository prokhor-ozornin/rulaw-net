...
/// </summary>
/// <seealso cref="IVotesApiExtensions"/>
public sealed class IVotesApiExtensionsTest : IntegrationTest
{
  ...
  public void Search_Methods()
  {
    using (new AssertionScope())
    {
      ...
      Test(Api.Votes.Search(new VotesSearchApiRequest().FromDate(DateTimeOffset.UtcNow.Subtract(TimeSpan.FromDays(180))).ToDate(DateTimeOffset.UtcNow).Deputy(99111987)));
    }
    ...
    using (new AssertionScope())
    {
      ...
      Test(Api.Votes.Search(request => request.FromDate(DateTimeOffset.UtcNow.Subtract(TimeSpan.FromDays(180))).ToDate(DateTimeOffset.UtcNow).Deputy(99111987)));
    }

    return;

    static void Test(IVotesSearchResult result)
    {
  ...
  public void SearchAsync_Method()
  {
    using (new AssertionScope())
    {
      ...
      Test(Api.Votes.SearchAsync(request => request.FromDate(DateTimeOffset.UtcNow.Subtract(TimeSpan.FromDays(180))).ToDate(DateTimeOffset.UtcNow).Deputy(99111987)));
    }

    return;

    static void Test(Task<IVotesSearchResult> task)
    {
