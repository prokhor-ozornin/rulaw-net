...
public sealed class LawTest : Test
{
  ...
  /// </summary>
  [Fact]
  public void Id_Property() => new Law { Id = long.MaxValue }.Id.Should().Be(long.MaxValue);
  
  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Name"/> property.</para>
  /// </summary>
  [Fact]
  public void Name_Property() => new Law { Name = Guid.Empty.ToString() }.Name.Should().Be(Guid.Empty.ToString());

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Date"/> property.</para>
  /// </summary>
  [Fact]
  public void Date_Property() => new Law { Date = DateTimeOffset.MaxValue }.Date.Should().Be(DateTimeOffset.MaxValue);

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Number"/> property.</para>
  /// </summary>
  [Fact]
  public void Number_Property() => new Law { Number = Guid.Empty.ToString() }.Number.Should().Be(Guid.Empty.ToString());

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Subject"/> property.</para>
  /// </summary>
  [Fact]
  public void Subject_Property() => new LawSubject().With(subject => new Law { Subject = subject }.Subject.Should().BeSameAs(subject));
  
  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Type"/> property.</para>
  /// </summary>
  [Fact]
  public void Type_Property() => new LawType().With(type => new Law { Type = type }.Type.Should().BeSameAs(type));

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Url"/> property.</para>
  /// </summary>
  [Fact]
  public void Url_Property() => new Law { Url = "http://yandex.ru" }.Url.Should().Be("http://yandex.ru");

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.TranscriptUrl"/> property.</para>
  /// </summary>
  [Fact]
  public void TranscriptUrl_Property() => new Law { TranscriptUrl = "http://yandex.ru" }.TranscriptUrl.Should().Be("http://yandex.ru");
  
  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Comments"/> property.</para>
  /// </summary>
  [Fact]
  public void Comments_Property() => new Law { Comments = Guid.Empty.ToString() }.Comments.Should().Be(Guid.Empty.ToString());

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.LastEvent"/> property.</para>
  /// </summary>
  [Fact]
  public void LastEvent_Property() => new LawEvent().With(lawEvent => new Law { LastEvent = lawEvent }.LastEvent.Should().BeSameAs(lawEvent));

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.Committees"/> property.</para>
  /// </summary>
  [Fact]
  public void Committees_Property() => new LawCommittees().With(committees => new Law { Committees = committees }.Committees.Should().BeSameAs(committees));

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.CompareTo(ILaw)"/> method.</para>
  /// </summary>
  [Fact]
  public void CompareTo_Method() => TestCompareTo<Law, DateTimeOffset>(nameof(Law.Date), DateTimeOffset.MinValue, DateTimeOffset.MaxValue); 
  ...
  /// </summary>
  [Fact]
  public void Equals_Methods() => TestEquality<Law, long>(nameof(Law.Id), 1, 2); 

  /// <summary>
  ///   <para>Performs testing of <see cref="Law.GetHashCode()"/> method.</para>
  /// </summary>
  [Fact]
  public void GetHashCode_Method() => TestHashCode<Law, long>(nameof(Law.Id), 1, 2); 
