...
public sealed class LawEventStageTest : Test
{
  /// <summary>
  ...
  /// </summary>
  [Fact]
  public void Id_Property() => new LawEventStage { Id = long.MaxValue }.Id.Should().Be(long.MaxValue);

  /// <summary>
  ///   <para>Performs testing of <see cref="LawEventStage.Name"/> property.</para>
  /// </summary>
  [Fact]
  public void Name_Property() => new LawEventStage { Name = Guid.Empty.ToString() }.Name.Should().Be(Guid.Empty.ToString());

  /// <summary>
  ///   <para>Performs testing of <see cref="LawEventStage.CompareTo(ILawEventStage)"/> method.</para>
  /// </summary>
  [Fact]
  public void CompareTo_Method() => TestCompareTo< LawEventStage, string>(nameof(LawEventStage.Name), "first", "second"); 
  ...
  /// </summary>
  [Fact]
  public void Equals_Methods() => TestEquality<LawEventStage, long>(nameof(LawEventStage.Id), 1, 2); 

  /// <summary>
  ///   <para>Performs testing of <see cref="LawEventStage.GetHashCode()"/> method.</para>
  /// </summary>
  [Fact]
  public void GetHashCode_Method() => TestHashCode<LawEventStage, long>(nameof(LawEventStage.Id), 1, 2); 
