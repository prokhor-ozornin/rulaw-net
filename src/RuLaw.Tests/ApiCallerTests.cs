...
namespace RuLaw
{
  ...
  public sealed class ApiCallerTests
  {
    ...
    public void Call_Methods()
    {
      ...
      try
      {
        caller.Call(string.Format(CultureInfo.InvariantCulture, "/{0}/deputy", caller.ApiToken));
      ...
      Assert.Equal("null", caller.Call(string.Format(CultureInfo.InvariantCulture, "/{0}/deputy", caller.ApiToken)).Content);

      const long deputyId = 99100142;

      var response = caller.Call(string.Format(CultureInfo.InvariantCulture, "/{0}/deputy", caller.ApiToken, deputyId), new { Id = deputyId });
      ...
      Assert.Equal(string.Format(CultureInfo.InvariantCulture, "http://api.duma.gov.ru/api/{0}/deputy.json?Id={2}&app_token={1}", caller.ApiToken, caller.AppToken, deputyId), response.ResponseUri.ToString());
      ...
      response = caller.Call(string.Format(CultureInfo.InvariantCulture, "/{0}/deputy", caller.ApiToken, deputyId), new { Id = deputyId });
      Assert.False(response.Content.IsEmpty());
      ...
      Assert.True(response.Cookies.Count > 0);
      Assert.True(response.Headers.Count > 0);
      Assert.Equal(string.Format(CultureInfo.InvariantCulture, "http://api.duma.gov.ru/api/{0}/deputy.xml?Id={2}&app_token={1}", caller.ApiToken, caller.AppToken, deputyId), response.ResponseUri.ToString());
      Assert.Equal(HttpStatusCode.OK, response.StatusCode);
      Assert.Equal("OK", response.StatusDescription);
      ...
      try
      {
        caller.Call<DeputyInfo>(string.Format(CultureInfo.InvariantCulture, "/{0}/deputy", caller.ApiToken));
        throw new InvalidOperationException();
      }
      catch (RuLawException exception)
      {
      ...
      Assert.Equal("null", caller.Call<DeputyInfo>(string.Format(CultureInfo.InvariantCulture, "/{0}/deputy", caller.ApiToken)).Content);

      response = caller.Call<DeputyInfo>(string.Format(CultureInfo.InvariantCulture, "/{0}/deputy", caller.ApiToken, deputyId), new { Id = deputyId });
      Assert.False(response.Content.IsEmpty());
      Assert.Equal("application/json", response.ContentType);
      Assert.Null(response.ErrorException);
      Assert.Null(response.ErrorMessage);
