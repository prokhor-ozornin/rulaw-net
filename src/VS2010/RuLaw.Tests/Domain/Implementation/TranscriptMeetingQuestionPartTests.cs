...
namespace RuLaw
{
  ...
  public sealed class TranscriptMeetingQuestionPartTests : UnitTestsBase<TranscriptMeetingQuestionPart>
  {
    ...
    public void Constructors()
    {
      ...
      Assert.False(part.LinesOriginal.Any());
      Assert.False(part.Lines.Any());
      Assert.Equal(0, part.StartLine);
      Assert.False(part.VotesOriginal.Any());
    ...
    {
      Assert.Equal("first{0}second".FormatSelf(Environment.NewLine), new TranscriptMeetingQuestionPart { LinesOriginal = new List<string> { "first", "second" } }.ToString());
    }
  }
}