...
public static class IActiveExtensions
{
  ...
  public static IEnumerable<TEntity> Active<TEntity>(this IEnumerable<TEntity> entities) where TEntity : IActive => entities?.Where(entity => entity is {Active: true}) ?? throw new ArgumentNullException(nameof(entities));
  ...
  /// <exception cref="ArgumentNullException">If <paramref name="entities"/> is <see langword="null"/>.</exception>
  public static IEnumerable<TEntity> Inactive<TEntity>(this IEnumerable<TEntity> entities) where TEntity : IActive => entities?.Where(entity => entity is {Active: false}) ?? throw new ArgumentNullException(nameof(entities));
}