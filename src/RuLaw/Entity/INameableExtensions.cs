...
public static class INameableExtensions
{
  ...
  public static IEnumerable<TEntity> Name<TEntity>(this IEnumerable<TEntity> entities, string name) where TEntity : INameable => entities?.Where(entity => entity is not null && entity.Name == name) ?? throw new ArgumentNullException(nameof(entities));
}