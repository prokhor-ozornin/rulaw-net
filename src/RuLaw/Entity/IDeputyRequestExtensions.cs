...
public static class IDeputyRequestExtensions
{
  ...
  public static IEnumerable<TEntity> Initiator<TEntity>(this IEnumerable<TEntity> requests, string initiator) where TEntity : IDeputyRequest => requests?.Where(request => request is not null && request.Initiator.ToInvariantString().Contains(initiator.ToInvariantString())) ?? throw new ArgumentNullException(nameof(requests));
  ...
  /// <exception cref="ArgumentNullException">If <paramref name="requests"/> is <see langword="null"/>.</exception>
  public static IEnumerable<TEntity> Answer<TEntity>(this IEnumerable<TEntity> requests, string text) where TEntity : IDeputyRequest => requests?.Where(request => request is not null && request.Answer.ToInvariantString().Contains(text.ToInvariantString())) ?? throw new ArgumentNullException(nameof(requests));
